...
## CRITICAL: 학습 파일 자동 prepend (QA/customer 페르소나 호출 공통)

페르소나 도우미(`harness-customer-user` · `harness-qa-engineer`) 호출 시 **공용 학습 파일 prepend 필수** (2026-05-20 정합화 — 프로젝트 learning 폐기). 외부 리서치는 페르소나 도우미가 아니며 shared `$deepresearch` skill 을 사용한다.
...
- 호출자 Codex가 페르소나 자리를 직접 수행하는 fallback 은 금지한다. `harness-qa-engineer` / `harness-customer-user` sub-agent/helper 도구가 없으면 `BLOCKED / DEPENDENCY_MISSING` 으로 기록하고 다음 step 에 진입하지 않는다.
...
| 결정 지점 | 위치 | 기본 동작 | 비고 |
|----------|------|----------|------|
| 도메인 설계 skill 선택 | step2 1번 | `harness-plan` skill (noask — 6 카테고리 합리적 가정 + 필요 시 shared `$deepresearch` 외부 리서치 + Open Questions 누적) | `/harness-ask` 모드는 `harness-plan-ask`. 외부 리서치 판단과 Phase 3·4·5 공유. [step2-domain.md](docs/steps/step2-domain.md) |
