...
**4. UNKNOWN (신규 — self-PASS bias 차단)**:
   - `fallback_used = manual self-test` (자동화 도구 없는데 호출자 Codex 가 코드만 보고 판단) **AND** 라벨 = PASS → **자동 UNKNOWN 강등** + 슬러그 `paused-by-unknown` 마킹
   - `harness-qa-engineer` helper/sub-agent 호출이 *없었던* 채로 (skill/helper/helper/sub-agent 호출 0회) PASS 라벨 → 자동 UNKNOWN 강등
...
- fallback_used: harness-qa-engineer sub-agent | manual self-test | none
...
- **harness-qa-engineer helper/sub-agent 호출 0회 + 라벨 = PASS 이면 → 자동 UNKNOWN 강등**. (도우미 우회 차단.)
