...
4. **harness-* helper 가용성 확인 + 안전 폴백** — 본 워크플로우가 사용할 harness-* helper 또는 Codex skill 가용성을 확인하고, **부재 항목은 안전 폴백 경로로 대체**한다. 자동 복구를 약속하지 않는다.
   ...
   | `harness-deep-researcher` | `~/.codex/skills/harness-deep-researcher/` | 부재 시 호출자 Codex가 직접 웹 검증 절차 수행 |
   | `harness-customer-user` | `~/.codex/skills/harness-customer-user/` | 부재 시 호출자 Codex가 고객 사용자 관점 절차를 직접 수행 |
...
5. **learning 파일 확인 (페르소나 3개)** — 다음 공용 learning 파일이 설치본에 있는지 확인한다. 없으면 helper를 호출한 것으로 간주하지 말고 호출자 Codex가 직접 수행하며 `Learning Prepend: not-used`를 기록한다.
   - `agents/learning/harness-customer-user.md`
   - `agents/learning/harness-qa-engineer.md`
   - `agents/learning/harness-deep-researcher.md`
