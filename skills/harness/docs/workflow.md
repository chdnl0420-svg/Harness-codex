...
**Codex fallback 시 주의:** Codex 인증 실패(exit 2) 또는 quota 소진(exit 3) 시 `code-review` skill 또는 `code-reviewer` agent 로 전환. 이 경우 **Codex가 구현·리뷰 모두 수행** → self-review bias 제거 효과 사라짐. 호출자 Codex 는 `fallback_used` 필드 명시 (workflow.md "회송 경로 실행 보장" 7필드 양식 참조), self-review LGTM:YES 는 자동으로 `LGTM:UNKNOWN` 강등.
